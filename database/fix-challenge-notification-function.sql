-- Fix Challenge Notification Function - Remove updated_at column

-- Drop and recreate the function without updated_at
DROP FUNCTION IF EXISTS create_challenge_notifications(uuid, text, uuid);

CREATE OR REPLACE FUNCTION create_challenge_notifications(
    p_challenge_id uuid,
    p_notification_type text, -- 'challenge_accepted'
    p_initiator_user_id uuid  -- The user who triggered this (accepter)
) RETURNS json AS $$
DECLARE
...
BEGIN
    ...
    -- Handle challenge accepted - create contact sharing notifications
